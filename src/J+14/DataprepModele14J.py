...
class DataPreparation(object):
    ...
    def __init__(self, directory, nombre_jours_futurs):
        ...
        self.data ={}
        self.prepared = {}
        self.nombre_jours_futurs = nombre_jours_futurs
        
        for i in range(1,self.nombre_jours_futurs):
            self.prepared[i] = pd.DataFrame()
    ...
    def PreparationElectricite(self):
        ...
        self.PreparationJourFutur(self.data[self.names_data[index_ref]]) 
        
    def PreparationJourFutur(self, conso):
        
        for j in range(1,self.nombre_jours_futurs):
            
            self.PreparationLag(conso, j)
            if j ==1:
                print(list(self.conso.columns))
                print("\n")
                
            print(" jour " + str(j) + " data prep has ended successfully ; saved in" + self.directory  + "\n")        
            
            pickle.dump(self.conso, open(self.directory + "/prepared/data_prepared_pred_conso_" + str(j) +".pkl", "wb" ) )

            self.prepared[j] = self.conso
            gc.collect()
        
    def PreparationLag(self, conso, jour):
        
        ##### gestion des lags de temperature
        ##### 2 cas de figure: températures trihoraires ou horaire ou températures quotidiennes.....

        if jour >=7:
            conso['temperature'] = conso['temperature'] +  np.random.randn(len(conso))
        ...
        for j in range(1,3): ###on prend la température moyenne, min et max des deux jours précédents
            conso['temperature_MeanJ-'+ str(j)] = conso['Mean_temperature'].shift(24*j)
            conso['temperature_MaxJ-'+ str(j)] = conso['Max_temperature'].shift(24*j)
            conso['temperature_MinJ-'+ str(j)] = conso['Min_temperature'].shift(24*j)
...
#        Lags = [1,2,3,4,5,6,7,10,11,12,14,16,18,21,24,25,26,29,33, 39,48,49,50,51,52,\
#                72,73,75,78,96,97,99,124,125,127,167,168,193,194,198,199,220,240,241,335,336,337]   
...
dd =  DataPreparation(directory+ "Data/", 2) 
print("execution time : " + str((time.time() - t0)/60.0) ) 
